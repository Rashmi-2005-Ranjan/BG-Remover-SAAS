...
const Result = () => {
    ...
    // Fix potential typo "date:" -> "data:"
    const safeResultImage =
        resultImage && resultImage.startsWith("date:")
            ? resultImage.replace("date:", "data:")
            : resultImage;
            ...
            <div className="bg-white rounded-lg px-8 py-6 shadow-md">
                            ...
                            {safeResultImage ? (
                                ...
                                image && (
                                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                        <div className="border-indigo-600 border-4 rounded-full h-12 w-12 border-t-transparent animate-spin"></div>
                                    </div>
                                )
                            )}
                        </div>
                ...
                {safeResultImage && (
                    <div className="flex justify-center sm:justify-end items-center flex-wrap gap-4 mt-6">
                        <button
                            className="border cursor-pointer text-indigo-600 font-semibold py-2 px-4 rounded-full text-lg hover:scale-105 transition-all duration-300"
                            onClick={() => navigate("/")}
                        >
                            Try Another Image
                        </button>
                        <a
                            href={safeResultImage}
                            download
